...
import "./interfaces/IDystopiaFactory.sol";
...
contract DystPair is IERC20, IDystopiaPair, Reentrancy {
  ...
  constructor() {
    factory = msg.sender;
    treasury = IDystopiaFactory(msg.sender).treasury();
    (address _token0, address _token1, bool _stable) = IDystopiaFactory(msg.sender).getInitializable();
  ...
  function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
    require(!IDystopiaFactory(factory).isPaused(), "DystPair: PAUSE");
