...
import {
  ...
  CurveSwapper128__factory,
  CurveSwapper256__factory,
  MockToken,
  ProxyControlled,
  ...
  KyberSwapper__factory, BalancerLinearPoolSwapper__factory, Pancake3Swapper__factory
} from "../../typechain";
...
export class DeployerUtils {
  ...
  public static async deployPancake3Swapper(signer: SignerWithAddress, controller: string) {
    const proxy = await DeployerUtils.deployProxy(signer, 'Pancake3Swapper')
    const swapper = Pancake3Swapper__factory.connect(proxy, signer);
    await RunHelper.runAndWait(() => swapper.init(controller, {gasLimit: 8_000_000}))
    return swapper;
  }
  ...
  public static async deployKyberSwapper(signer: SignerWithAddress, controller: string) {
    const proxy = await DeployerUtils.deployProxy(signer, 'KyberSwapper')
    const swapper = KyberSwapper__factory.connect(proxy, signer);
    await RunHelper.runAndWait(() => swapper.init(controller, {gasLimit: 8_000_000}))
    return swapper;
  }
  ...
  public static async deployCurveSwapper128(signer: SignerWithAddress, controller: string) {
    const proxy = await DeployerUtils.deployProxy(signer, 'CurveSwapper128')
    const swapper = CurveSwapper128__factory.connect(proxy, signer);
    await RunHelper.runAndWait(() => swapper.init(controller, {gasLimit: 8_000_000}))
    return swapper;
  }

  public static async deployCurveSwapper256(signer: SignerWithAddress, controller: string) {
    const proxy = await DeployerUtils.deployProxy(signer, 'CurveSwapper256')
    const swapper = CurveSwapper256__factory.connect(proxy, signer);
    await RunHelper.runAndWait(() => swapper.init(controller, {gasLimit: 8_000_000}))
    return swapper;
  }
